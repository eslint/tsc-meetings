...
safe-outputs:
  # gh-aw's default footer refers to the triggering issue as a bare "#<number>",
  # which GitHub resolves against the repository the comment is posted in. Since
  # this workflow comments on issues/PRs across the eslint org, that produced
  # links to unrelated issues in the target repo. Qualify the reference with the
  # repository so it always points back here.
  messages:
    footer: "> Generated by [{workflow_name}]({run_url}) for ${{ github.repository }}#{triggering_number}{ai_credits_suffix}{history_link}"
  create-pull-request:
    title-prefix: "Add "
    labels: [automation]
    max: 1
