...
const now = moment();
const firstDayOfPreviousMonth = now
	.clone()
	.subtract(1, "month")
	.startOf("month");
...
async function fetchMonthlyDonations() {
	const url =
		"https://raw.githubusercontent.com/eslint/eslint.org/refs/heads/main/src/_data/sponsors.json";
	...
	if (!response.ok) {
		...
		throw new Error(
			`Failed to fetch sponsors data with status ${response.status}`,
		);
	}
	...
	if (
		!data ||
		!data.totals ||
		typeof data.totals.monthlyDonations !== "number"
	) {
		throw new Error(
			"Invalid sponsors data structure: missing totals.monthlyDonations",
		);
	}
...
async function fetchGitHubSearchResults() {
	...
	while (hasMorePages) {
		...
		console.log(
			`Retrieved ${allItems.length} of ${totalCount} total results`,
		);
	}

	console.log(`Fetched ${allItems.length} PRs total\n`);

	return allItems.map(pr => {
		const smallPr = {};
...
(async () => {
	...
	const contributorPoolFunds =
		Math.floor((monthlyDonations / CONTRIBUTOR_POOL_DIVISOR) * 100) / 100;
